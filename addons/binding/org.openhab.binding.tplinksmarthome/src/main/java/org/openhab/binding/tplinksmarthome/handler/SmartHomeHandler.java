...
public class SmartHomeHandler extends BaseThingHandler {
    ...
    public void handleCommand(ChannelUID channelUID, Command command) {
        try {
            if (command instanceof RefreshType) {
                updateChannelState(channelUID, getCacheValue());
    ...
    public void initialize() {
        ...
        cache = new ExpiringCache<DeviceState>(TimeUnit.SECONDS.toMillis(configuration.refresh), () -> {
            try {
                return new DeviceState(connection.sendCommand(smartHomeDevice.getUpdateCommand()));
            } catch (IOException e) {
                logger.warn("TP-Link SmartHome Initialize error", e);
                updateStatus(ThingStatus.OFFLINE, ThingStatusDetail.OFFLINE.COMMUNICATION_ERROR, e.getMessage());
     ...
     *
     * @param config configuration ot be used by the connection
    ...
    private void startAutomaticRefresh(TPLinkSmartHomeConfiguration config) {
        if (refreshJob == null || refreshJob.isCancelled()) {
            Runnable runnable = () -> {
                ...
                DeviceState value = getCacheValue();
    ...
    /**
     * Returns the cached device or new value and update state depending if not available.
     *
     * @return actual device state
     */
    private DeviceState getCacheValue() {
        synchronized (cache) {
            DeviceState value = cache.getValue();

            updateStatus(value == null ? ThingStatus.OFFLINE : ThingStatus.ONLINE);
            return value;
        }
    }
}
