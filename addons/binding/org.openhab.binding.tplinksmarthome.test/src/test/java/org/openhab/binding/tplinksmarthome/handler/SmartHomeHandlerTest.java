...
public class SmartHomeHandlerTest {
    ...
    public void initializeShouldCallTheCallback() throws InterruptedException {
        ...
        assertEquals("", ThingStatus.OFFLINE, thingStatusInfo.getStatus());
    }
    ...
    public void testHhandleCommandRefreshType() {
        ...
        assertEquals("", new DecimalType(-53), stateCaptor.getValue());
    }
    ...
    public void testHhandleCommandOther() throws InterruptedException {
        ...
        ArgumentCaptor<State> stateCaptor = ArgumentCaptor.forClass(State.class);
        verify(callback).stateUpdated(eq(channelUID), stateCaptor.capture());
        assertSame("", OnOffType.ON, stateCaptor.getValue());
    }
}
