 ...
 * @author Hilbrand Bouwkamp - Reorganized code, detecting things runs in same thread, sys info is put into properties
 */
...
public class TPLinkSmartHomeDiscoveryService extends AbstractDiscoveryService {
    ...
    private static final long DISCOVERY_REFRESH_INTERVAL = 60 * 10; // every 10 minutes
    ...
    protected void startBackgroundDiscovery() {
        discoveryJob = scheduler.scheduleWithFixedDelay(() -> startScan(), 0, DISCOVERY_REFRESH_INTERVAL,
                TimeUnit.SECONDS);
    }
    ...
    /**
     * Opens a {@link DatagramSocket} and sends a packet for discovery of TP-Link Smart devices.
     *
     * @return Returns the new socket
     * @throws IOException
     */
    protected DatagramSocket startDiscoverSocket() throws IOException {
    ...
    private void detectThing(DatagramPacket packet) throws IOException {
        ...
        Optional<@NonNull ThingTypeUID> thingTypeUID = getThingTypeUID(sysinfo.getModel());
     ...
     */
    private Optional<@NonNull ThingTypeUID> getThingTypeUID(String model) {
