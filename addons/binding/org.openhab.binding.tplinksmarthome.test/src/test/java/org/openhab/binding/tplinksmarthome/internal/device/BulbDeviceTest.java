...
public class BulbDeviceTest extends DeviceTestBase {
    ...
    public void testHandleCommandColorTemperature() throws IOException {
        ...
        assertTrue("Color temperature channel should be handled",
                device.handleCommand(CHANNEL_COLOR_TEMPERATURE, connection, new DecimalType(5100), configuration));
    }
    ...
    @Test
    public void testHandleCommandColorTemperaturePerc() throws IOException {
        assertInput("bulb_transition_light_state_color_temp");
        assertTrue("Color temperature percenatage channel should be handled", device
                .handleCommand(CHANNEL_COLOR_TEMPERATURE_PERCENTAGE, connection, new PercentType(40), configuration));
    }

    @Test
    public void testHandleCommandColorTemperaturePercOnOff() throws IOException {
        assertInput("bulb_transition_light_state_on");
        assertTrue("Color temperature percenatage channel with OnOff state should be handled",
                device.handleCommand(CHANNEL_COLOR_TEMPERATURE_PERCENTAGE, connection, OnOffType.ON, configuration));
    }
